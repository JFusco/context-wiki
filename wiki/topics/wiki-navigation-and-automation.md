...
- `scripts/wiki/routing-policy.json` is shared by the CLI and browser. Routes use explicit edge costs, hub and byte penalties, stable tie-breaking, and exact Source/Target IDs.
- Agent and browser output identifies explicit Source→Target authority, each page relationship and byte count, and total bytes. Search includes repository-qualified GitHub evidence. Agents read the itinerary in order, stop when grounded, and resolve ambiguity with returned candidates or one targeted search.
...
- Merge reconciliation is event-driven and manually replayable. It merges every closing-keyword same- or cross-repository issue into an existing journal, ignores arbitrary and fenced citations, and drafts a journal only when none exists. Only issue-state refresh needs the daily `30 11 * * *` schedule because an external issue can close or reopen without a repository event; refresh tracks Markdown fence state so fenced headings and URLs never change section scope or trigger lookups.
